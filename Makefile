...
packspec: ## Regenerate runtime/prompt/packspec from the embedded PromptPack schema
	@go -C tools/packspec-gen run . \
		-schema ../../runtime/prompt/schema/promptpack.schema.json \
		-out ../../runtime/prompt/packspec/types.go

packspec-check: ## Check the generated pack types are current (for CI)
	@go -C tools/packspec-gen run . \
		-schema ../../runtime/prompt/schema/promptpack.schema.json \
		-out ../../runtime/prompt/packspec/types.go -check
...
docs-reference: ## Generate per-package runtime reference pages from Go source
	@echo "📖 Generating runtime reference..."
	@chmod +x docs/scripts/gen-reference.sh
