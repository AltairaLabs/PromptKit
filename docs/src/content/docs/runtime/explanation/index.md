...
pipeline: Pipeline {
  statestore: StateStoreLoad Stage (load conversation)
  prompt: PromptAssembly Stage (apply templates)
  validation: Validation Stage (check content)
  provider_stage: Provider Stage (call LLM) {
