...
- [type OTelEventListener](<#OTelEventListener>)
  - [func NewOTelEventListener\(tracer trace.Tracer, opts ...OTelOption\) \*OTelEventListener](<#NewOTelEventListener>)
...
- [type OTelOption](<#OTelOption>)
  - [func WithContentCapture\(enabled bool\) OTelOption](<#WithContentCapture>)
  - [func WithRedactor\(r Redactor\) OTelOption](<#WithRedactor>)
...
Enable this only where the trace backend is an appropriate place for customer data. Pair it with WithRedactor where tool arguments may carry credentials: on\-behalf\-of token exchange puts per\-user delegated OAuth tokens into tool arguments, and those would otherwise land in the trace verbatim.

<a name="WithRedactor"></a>
### func WithRedactor
...
WithRedactor sets the scrubber applied to content attributes when capture is enabled. It has no effect while capture is off, because nothing is recorded to scrub.
...
Called once per attribute, with the attribute key \(e.g. "gen\_ai.tool.call.arguments"\) and the value that would otherwise be recorded. Return the value to record; return "" to omit the attribute entirely.

Only invoked when content capture is ENABLED — it is a scrubber for deliberately\-captured content, not a substitute for the gate. A nil Redactor records values unchanged, which is what enabling capture without one asks for.
