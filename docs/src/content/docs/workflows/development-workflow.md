...
build: 1. Build {
  main: Create main.go with Runtime
  pipeline: Set up pipeline with middleware
  logic: Implement business logic
}

test: 2. Test {
  cases: Write arena.yaml test cases
  run: Run promptarena run
  iterate: Iterate based on results
}

organize: 3. Organize {
  extract: Extract prompts to files
  pack: Package with PackC
  load: Load from .pack files
}
...
build -> test -> organize -> iter -> package -> validate
```
...
- **Deploy to production**: See [Deployment Workflow](deployment-workflow)
- **Add more tests**: See [Testing Workflow](testing-workflow)
- **Build full-stack app**: See [Full-Stack Example](full-stack-example)

## Related Documentation

- [Runtime Getting Started](../runtime/tutorials/01-first-pipeline)
- [PromptArena Configuration](../promptarena/configuration)
- [PackC Commands](../packc/commands)
- [Production Deployment](../runtime/tutorials/05-production-deployment)
