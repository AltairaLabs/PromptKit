...
```yaml
# Invalid Go template
user_template: "{{.name"  # Missing closing }}
```
...
user_template: "{{range .items}}{{.}}{{end}}"

# Invalid
user_template: "{{.name"           # Missing }}
user_template: "{{undefined .x}}"  # Unknown function
```
...
user_template: "{{.mystery_var}}"  # ⚠️ Undocumented variable
```
