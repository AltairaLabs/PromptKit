...
func TestSelfPlayRegistry_GetTTSRegistry(t *testing.T) {
	...
	if ttsReg == nil {
		t.Error("Expected TTS registry to be initialized")
	}
}

func TestNewRegistryWithTTS_NilTTSRegistry(t *testing.T) {
	registry := NewRegistryWithTTS(providers.NewRegistry(), nil, nil, nil, nil)

	// Should still have a default TTS registry
	ttsReg := registry.GetTTSRegistry()
	if ttsReg == nil {
