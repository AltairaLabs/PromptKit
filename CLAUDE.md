...
- Check new tests can actually fail (`scripts/check-weak-assertions.sh`, staged `_test.go` only) — mirrors CI's Weak Assertion Guard, but based on `HEAD` so it flags only what this commit adds
