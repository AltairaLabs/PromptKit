...
type Collector struct {
	...
	// TTS metric families
	ttsRequestDuration   *prometheus.HistogramVec
	ttsRequestsTotal     *prometheus.CounterVec
	ttsCharactersTotal   *prometheus.CounterVec
	ttsAudioSecondsTotal *prometheus.CounterVec
	ttsCostTotal         *prometheus.CounterVec

	// STT metric families
	sttRequestDuration   *prometheus.HistogramVec
	sttRequestsTotal     *prometheus.CounterVec
	sttAudioSecondsTotal *prometheus.CounterVec
	sttCostTotal         *prometheus.CounterVec

	// Embedding metric families
	embeddingRequestDuration  *prometheus.HistogramVec
	embeddingRequestsTotal    *prometheus.CounterVec
	embeddingInputTokensTotal *prometheus.CounterVec
	embeddingVectorsTotal     *prometheus.CounterVec
	embeddingCostTotal        *prometheus.CounterVec

	// Image gen metric families
	imageGenRequestDuration *prometheus.HistogramVec
	imageGenRequestsTotal   *prometheus.CounterVec
...
func (c *Collector) registerPipelineMetrics() {
	...
	// TTS
	c.ttsRequestDuration = c.mustRegisterHistogramVec(
		"tts_request_duration_seconds",
		"Duration of TTS provider calls in seconds",
		providerBuckets,
		[]string{"provider", "model", "source"},
	)
	c.ttsRequestsTotal = c.mustRegisterCounterVec(
		"tts_requests_total",
		"Total number of TTS provider calls",
		[]string{"provider", "model", "source", "status"},
	)
	c.ttsCharactersTotal = c.mustRegisterCounterVec(
		"tts_characters_total",
		"Total characters synthesized by TTS providers",
		[]string{"provider", "model", "source"},
	)
	c.ttsAudioSecondsTotal = c.mustRegisterCounterVec(
		"tts_audio_seconds_total",
		"Total audio seconds generated by TTS providers",
		[]string{"provider", "model", "source"},
	)
	c.ttsCostTotal = c.mustRegisterCounterVec(
		"tts_cost_total",
		"Total cost in USD from TTS provider calls",
		[]string{"provider", "model", "source"},
	)

	// STT
	c.sttRequestDuration = c.mustRegisterHistogramVec(
		"stt_request_duration_seconds",
		"Duration of STT provider calls in seconds",
		providerBuckets,
		[]string{"provider", "model", "source"},
	)
	c.sttRequestsTotal = c.mustRegisterCounterVec(
		"stt_requests_total",
		"Total number of STT provider calls",
		[]string{"provider", "model", "source", "status"},
	)
	c.sttAudioSecondsTotal = c.mustRegisterCounterVec(
		"stt_audio_seconds_total",
		"Total audio seconds transcribed by STT providers",
		[]string{"provider", "model", "source"},
	)
	c.sttCostTotal = c.mustRegisterCounterVec(
		"stt_cost_total",
		"Total cost in USD from STT provider calls",
		[]string{"provider", "model", "source"},
	)

	// Embedding
	c.embeddingRequestDuration = c.mustRegisterHistogramVec(
		"embedding_request_duration_seconds",
		"Duration of embedding provider calls in seconds",
		providerBuckets,
		[]string{"provider", "model", "source"},
	)
	c.embeddingRequestsTotal = c.mustRegisterCounterVec(
		"embedding_requests_total",
		"Total number of embedding provider calls",
		[]string{"provider", "model", "source", "status"},
	)
	c.embeddingInputTokensTotal = c.mustRegisterCounterVec(
		"embedding_input_tokens_total",
		"Total input tokens sent to embedding providers",
		[]string{"provider", "model", "source"},
	)
	c.embeddingVectorsTotal = c.mustRegisterCounterVec(
		"embedding_vectors_total",
		"Total vectors returned by embedding providers",
		[]string{"provider", "model", "source"},
	)
	c.embeddingCostTotal = c.mustRegisterCounterVec(
		"embedding_cost_total",
		"Total cost in USD from embedding provider calls",
		[]string{"provider", "model", "source"},
	)

	// Image gen
	c.imageGenRequestDuration = c.mustRegisterHistogramVec(
		"image_gen_request_duration_seconds",
		"Duration of image generation calls in seconds",
		providerBuckets,
		[]string{"provider", "model", "source"},
	)
	c.imageGenRequestsTotal = c.mustRegisterCounterVec(
		"image_gen_requests_total",
		"Total number of image generation calls",
		[]string{"provider", "model", "source", "status"},
	)
	c.imageGenImagesTotal = c.mustRegisterCounterVec(
		"image_gen_images_total",
		"Total images generated",
		[]string{"provider", "model", "source"},
	)
	c.imageGenCostTotal = c.mustRegisterCounterVec(
		"image_gen_cost_total",
...
func (mc *MetricContext) OnEvent(event *events.Event) {
	...
	case events.EventImageGenCallCompleted:
...
func (mc *MetricContext) handleImageGenCallFailed(event *events.Event) {
	data, ok := event.Data.(*events.ImageGenCallFailedData)
	if !ok {
		return
	}
	exemplar := traceExemplar(event.SpanContext)
