...
func TestOpenAIToolProvider_WithMultimodalMessages(t *testing.T) {
	toolProvider := NewOpenAIToolProvider("test", "gpt-4o", "https://api.openai.com/v1", ProviderDefaults{}, false, nil)
...
func TestOpenAIToolProvider_MultimodalToolSupport(t *testing.T) {
	...
	// Check if it implements MultimodalToolSupport interface
	_, ok := interface{}(toolProvider).(MultimodalToolSupport)
	if !ok {
		t.Error("OpenAIToolProvider should implement MultimodalToolSupport interface")
		t.Log("    Missing: ChatMultimodalWithTools() method")
	} else {
		t.Log("✓ OpenAIToolProvider implements MultimodalToolSupport interface")
	}

	// Check if it at least implements MultimodalSupport
	_, ok = interface{}(toolProvider).(MultimodalSupport)
	if ok {
		t.Log("✓ OpenAIToolProvider implements MultimodalSupport interface")
		t.Log("    This means it inherits GetMultimodalCapabilities(), ChatMultimodal(), ChatMultimodalStream()")
	} else {
		t.Log("⚠️  OpenAIToolProvider does NOT implement MultimodalSupport interface")
	}
}
...
func TestOpenAIProvider_InheritsMultimodal(t *testing.T) {
	baseProvider := NewOpenAIProvider("test", "gpt-4o", "https://api.openai.com/v1", ProviderDefaults{}, false)

	// Check if base provider implements MultimodalSupport
	_, ok := interface{}(baseProvider).(MultimodalSupport)
	if !ok {
		t.Fatal("OpenAIProvider should implement MultimodalSupport interface")
	}
	...
	_, ok = interface{}(toolProvider).(MultimodalSupport)
	if !ok {
		t.Fatal("OpenAIToolProvider should inherit MultimodalSupport from OpenAIProvider")
	}

	t.Log("✓ OpenAIToolProvider inherits MultimodalSupport from OpenAIProvider")
}
...
func TestOpenAIToolProvider_MultimodalValidation(t *testing.T) {
	...
	// Verify ValidateMultimodalMessage also catches this
	err = ValidateMultimodalMessage(toolProvider, msg)
	if err == nil {
		t.Error("Expected ValidateMultimodalMessage to reject audio")
	} else {
		t.Log("✓ ValidateMultimodalMessage correctly rejects audio:", err)
	}
}
