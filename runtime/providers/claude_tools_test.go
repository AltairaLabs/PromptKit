...
func TestClaudeBuildTooling_Empty(t *testing.T) {
	provider := NewClaudeToolProvider("test", "claude-3-opus", "https://api.anthropic.com/v1", ProviderDefaults{}, false)
...
func TestClaudeParseToolResponse_NoToolCalls(t *testing.T) {
	...
	chatResp, toolCalls, err := provider.parseToolResponse([]byte(responseJSON), ChatResponse{})
	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}
...
func TestClaudeParseToolResponse_InvalidJSON(t *testing.T) {
	...
	_, _, err := provider.parseToolResponse([]byte(responseJSON), ChatResponse{})
	if err == nil {
...
func TestClaudeParseToolResponse_CachedTokens(t *testing.T) {
	...
	chatResp, _, err := provider.parseToolResponse([]byte(responseJSON), ChatResponse{})
	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}
