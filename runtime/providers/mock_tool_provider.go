...
func (m *MockToolProvider) BuildTooling(descriptors []*ToolDescriptor) (interface{}, error) {
...
func (m *MockToolProvider) ChatWithTools(ctx context.Context, req ChatRequest, tools interface{}, toolChoice string) (ChatResponse, []types.MessageToolCall, error) {
	...
	if err != nil {
		return ChatResponse{}, nil, fmt.Errorf("failed to get mock turn: %w", err)
	}
	...
	if mockTurn.Type == "tool_calls" {
		...
		for i, tc := range mockTurn.ToolCalls {
			...
			if err != nil {
				return ChatResponse{}, nil, fmt.Errorf("failed to marshal tool call arguments: %w", err)
			}
...
func (m *MockToolProvider) detectTurnFromConversation(req ChatRequest) int {
...
func (m *MockToolProvider) getScenarioID(req ChatRequest) string {
	if req.Metadata != nil {
