...
func TestOpenAIProvider_ID(t *testing.T) {
	...
	for _, id := range ids {
		provider := NewOpenAIProvider(id, "model", "url", ProviderDefaults{}, false)
		if provider.ID() != id {
...
func TestOpenAIProvider_DifferentModels(t *testing.T) {
	...
	for _, model := range models {
		provider := NewOpenAIProvider("test", model, "url", ProviderDefaults{}, false)
...
func TestOpenAIProvider_DifferentBaseURLs(t *testing.T) {
	...
	for _, url := range urls {
		provider := NewOpenAIProvider("test", "gpt-4", url, ProviderDefaults{}, false)
