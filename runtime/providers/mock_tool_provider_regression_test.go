...
// TestMockToolProvider_ContinueWithToolResults_Progression tests that turn progression
// works correctly when continuing with tool results.
func TestMockToolProvider_ContinueWithToolResults_Progression(t *testing.T) {
	configData := `
scenarios:
  multi-turn-scenario:
    turns:
      ...
      2:
        response: "Found user account. Let me check their subscription."
      3:
        tool_calls:
          - name: "get_subscription"
            arguments:
              user_id: "12345"
      4:
        response: "User has an active Pro subscription. How can I help you further?"
`

	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "multi-turn-config.yaml")
	err := os.WriteFile(configPath, []byte(configData), 0644)
	require.NoError(t, err)
	...
	// Start with turn 1 (tool calls)
	req := ChatRequest{
		Messages: []types.Message{
			{Role: "user", Content: "Look up my account: user@example.com"},
		},
		Metadata: map[string]interface{}{
			"mock_scenario_id": "multi-turn-scenario",
			"mock_turn_number": 1,
		},
	}

	response1, toolCalls1, err := provider.ChatWithTools(ctx, req, nil, "auto")
	require.NoError(t, err)
	require.Len(t, toolCalls1, 1)
	assert.Equal(t, "lookup_user", toolCalls1[0].Name)

	// Continue with tool results - should advance to turn 2
	mockResults := []types.MessageToolResult{
		{Name: "lookup_user", Content: `{"user_id": "12345", "name": "John Doe"}`},
	}

	response2, toolCalls2, err := provider.ContinueWithToolResults(ctx, req, response1, nil, "auto", mockResults)
	require.NoError(t, err)
	assert.Empty(t, toolCalls2, "Turn 2 should be a text response")
	assert.Contains(t, response2.Content, "Found user account")
	assert.Contains(t, response2.Content, "check their subscription")

	// Verify that the response is from turn 2 (indirectly through content)
	// Note: Metadata updates are handled internally, we verify behavior through response content
}
