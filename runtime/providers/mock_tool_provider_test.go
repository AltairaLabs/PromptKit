...
func TestMockToolProvider_ContinueWithToolResults(t *testing.T) {
	// Create repository with multi-turn response
	repo := NewInMemoryMockRepository("default")
	repo.SetResponse("test-scenario", 2, "Based on the weather data, it's sunny!")
	...
	// Previous response with tool calls
	prior := ChatResponse{
		Content: "Let me check the weather for you.",
		ToolCalls: []types.MessageToolCall{
			{ID: "call_1", Name: "get_weather", Args: json.RawMessage(`{"location": "SF"}`)},
		},
	}

	// Tool results
	results := []types.MessageToolResult{
		{
			ID:      "call_1",
			Name:    "get_weather",
	...
	ctx := context.Background()
	resp, toolCalls, err := provider.ContinueWithToolResults(ctx, req, prior, nil, "auto", results)

	require.NoError(t, err)
	assert.Equal(t, "Based on the weather data, it's sunny!", resp.Content)
	assert.Nil(t, toolCalls)
	assert.NotNil(t, resp.CostInfo)
}
...
func TestMockToolProvider_ContinueWithToolResults_MetadataIncrement(t *testing.T) {
	// Test that ContinueWithToolResults properly increments turn number
	repo := NewInMemoryMockRepository("default")
	repo.SetResponse("test-scenario", 2, "Turn 2 response")
	repo.SetResponse("test-scenario", 3, "Turn 3 response")

	provider := NewMockToolProviderWithRepository("test-id", "test-model", false, repo)

	// Test incrementing from turn 1 to turn 2
	req := ChatRequest{
		Messages: []types.Message{
	...
	prior := ChatResponse{Content: "Previous response"}
	results := []types.MessageToolResult{
		{ID: "call_1", Name: "tool_name", Content: "tool result"},
	}
	...
	require.NoError(t, err)
	assert.Equal(t, "Turn 2 response", resp.Content)
	assert.Nil(t, toolCalls)

	// Test incrementing from turn 2 to turn 3
	req2 := ChatRequest{
		Messages: []types.Message{
	...
	resp2, toolCalls2, err := provider.ContinueWithToolResults(ctx, req2, prior, nil, "auto", results)

	require.NoError(t, err)
	assert.Equal(t, "Turn 3 response", resp2.Content)
	assert.Nil(t, toolCalls2)
}

func TestMockToolProvider_ContinueWithToolResults_NoMetadata(t *testing.T) {
	// Test ContinueWithToolResults behavior when no turn metadata present
	repo := NewInMemoryMockRepository("fallback response")
	provider := NewMockToolProviderWithRepository("test-id", "test-model", false, repo)

	req := ChatRequest{
		Messages: []types.Message{
			{Role: "user", Content: "Request without metadata"},
		},
		// No metadata
	}
